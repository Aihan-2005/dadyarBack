...
export class TokenService {
  ...
  private signToken(payload: object, secret: string, expiresInSec: number, jti?: string) {
    return jwt.sign(payload, secret, { jwtid: jti, expiresIn: expiresInSec });
  }

  public generateAccessToken(userId: string): string {
    // 15 minutes
    return this.signToken({ userId } satisfies Payload, env.JWT_ACC_SECRET, 15 * 60);
  }
  ...
  public async generateRefreshToken(userId: string): Promise<string> {
    ...
    // 30 days in seconds
    const raw = this.signToken({ userId } satisfies Payload, env.JWT_REF_SECRET, 30 * 24 * 60 * 60, jti);
  ...
  public async rotateRefreshToken(refreshToken: string) {
    ...
    if (!jti || !userId) throw new HttpExceptoin(401, MESSAGES.invlaidRefToken[env.LANGUAGE]);

    const stored = await this.repo.findByJti(jti);
    if (!stored) throw new HttpExceptoin(401, MESSAGES.noRefToken[env.LANGUAGE]);
