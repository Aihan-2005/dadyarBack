...
import { CreateLawyerInput, Level, Skill, Study, WorkExperience } from "../interfaces/lawyer.interface"
...
export class LawyerService {
  ...
  public updateProfile(id: string, patch: Partial<Pick<CreateLawyerInput, "name" | "lastname" | "address" | "yearsOfExperience" | "website" | "bio">>) {
  ...
  public async removeStudy(lawyerId: string, studyId: string) {
    ...
    if (result.matchedCount === 0) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE])
    if (result.modifiedCount === 0) throw new HttpExceptoin(404, MESSAGES.noStudyWithId[LANGUAGE])
  }
  ...
  public async removeLanguage(lawyerId: string, language: string) {
    ...
    if (result.matchedCount === 0) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE])
    if (result.modifiedCount === 0) throw new HttpExceptoin(404, MESSAGES.noLanguageFound[LANGUAGE])
  }
  ...
  public async removeSkill(lawyerId: string, skillName: string) {
    ...
    if (result.matchedCount === 0) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE])
    if (result.modifiedCount === 0) throw new HttpExceptoin(404, MESSAGES.noSkillWithName[LANGUAGE])
  }

  public async changeSkillLevel(lawyerId: string, skillName: string, newLevel: Level) {
    const result = await this.repo.updateSkillLevel(lawyerId, skillName, newLevel);
    if (result.matchedCount === 0) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE])
    if (result.modifiedCount === 0) throw new HttpExceptoin(404, MESSAGES.noSkillWithName[LANGUAGE])
  }

  public async addWorkExperience(lawyerId: string, workExperience: WorkExperience) {
    const updated = await this.repo.addWorkExperience(lawyerId, workExperience);
    if (!updated) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE]);
    return updated;
  }

  public async removeWorkExperience(lawyerId: string, workExperienceId: string) {
    const result = await this.repo.removeStudy(lawyerId, workExperienceId);
    if (result.matchedCount === 0) throw new HttpExceptoin(404, MESSAGES.noUserWithId[LANGUAGE])
    if (result.modifiedCount === 0) throw new HttpExceptoin(404, MESSAGES.noWorkExperienceWithId[LANGUAGE])
  }
