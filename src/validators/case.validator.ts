...
import {
  OptionalLawyerClientIdentitySchema,
  OptionalNationalIdSchema,
} from "./lawyerClient.validator";
...
const LongOptionalString = cleanOptionalString(5000);
...
const MoneySchema = z.preprocess(
  (value) => {
    if (typeof value === "string" && value.trim() !== "") {
      return Number(
        normalizePersianDigits(value.trim())
          .replace(/[٬,\s]/g, "")
...
export const CasePaymentTypeSchema = z.enum(CASE_PAYMENT_TYPES);
...
export const CourtSchema = z
  .object({
    ...
    archiveNumberBranch: OptionalString,
  })
  .strict();

export const BranchHistorySchema = z
  .object({
    province: OptionalString,

    city: OptionalString,

    branchNumber: OptionalString,
...
export const ExistingCaseClientSchema = z
  .object({
    clientId: MongoIdSchema,
...
export const CaseRequestClientSchema = z.union([
  ExistingCaseClientSchema,
  ManualCaseClientSchema,
]);

export const OpposingPartySchema = z
  .object({
    fullName: RequiredString,

    phone: OptionalString,

    nationalId: OptionalIdentityDocumentSchema,
...
export const LawyerContactSchema = z
  .object({
    fullName: RequiredString,
    ...
    licenseExpiresAt: OptionalDateSchema,

    licensePlaceOfIssue: OptionalString,
  })
  .strict();

export const RelatedPersonSchema = z
  .object({
    fullName: RequiredString,
...
const CaseBodySchema = z
  .object({
    ...
    archiveNumberOffice: OptionalString,

    value: MoneySchema.optional().default(0),

    state: CaseStateSchema.optional(),

    description: LongOptionalString,

    paymentType: CasePaymentTypeSchema.optional(),

    nonCashDescription: OptionalString,
    ...
    clients: z.array(CaseRequestClientSchema).min(1, {
...
export const UpdateCaseStateSchema = z
  .object({
...
export const ParamCaseIdSchema = z
  .object({
    caseId: MongoIdSchema,
  })
  .strict();

export const ParamSubDocumentIdSchema = z
  .object({
    id: MongoIdSchema,
  })
  .strict();

export const ParamCaseAndOpposingPartyIdSchema = z
  .object({
    caseId: MongoIdSchema,
...
export const ParamCaseAndAssistantLawyerIdSchema = z
  .object({
    caseId: MongoIdSchema,
...
export const ParamCaseAndOpposingLawyerIdSchema = z
  .object({
    caseId: MongoIdSchema,
...
export const ParamCaseAndRelatedPersonIdSchema = z
  .object({
    caseId: MongoIdSchema,
