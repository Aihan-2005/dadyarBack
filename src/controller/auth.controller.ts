...
class AuthController {
  ...
  public signup = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { user, accessToken, refreshToken } = await this.authService.signup(parsed)
  ...
  public login = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const { user, accessToken, refreshToken } = await this.authService.login(parsed)
  ...
  public refresh = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!refreshToken) throw new HttpExceptoin(401, MESSAGES.unauthorized[LANGUAGE])
