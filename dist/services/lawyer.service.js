...
class LawyerService {
    ...
    async updateProfile(lawyerId, input) {
        ...
        if (!normalizedPhone &&
            !current.email) {
            throw new httpException_1.HttpExceptoin(400, messages_1.MESSAGES.noEmailNorPhone[LANGUAGE]);
        }
        const phoneChanged = normalizedPhone !==
            current.phone;
        const licenseChanged = normalizedLicense !==
        ...
        const [phoneOwner, licenseOwner,] = await Promise.all([
            ...
            normalizedLicense &&
                licenseChanged
                ? this.repo
                    .findByLicenseNumber(normalizedLicense)
                : Promise.resolve(null),
        ]);
        if (phoneOwner &&
            !this.sameLawyer(phoneOwner._id, lawyerId)) {
            throw new httpException_1.HttpExceptoin(409, messages_1.MESSAGES.phoneExsist[LANGUAGE]);
        }
        if (licenseOwner &&
            !this.sameLawyer(licenseOwner._id, lawyerId)) {
            throw new httpException_1.HttpExceptoin(409, messages_1.MESSAGES.barExsist[LANGUAGE]);
        }
        const setFields = {
        ...
        if (licenseChanged) {
            setFields.licenseVerifiedAt =
                null;
            if (current.status ===
                lawyer_constants_1.LAWYER_STATUSES.ACTIVE) {
                setFields.status =
                    lawyer_constants_1.LAWYER_STATUSES
        ...
        const updated = await this.repo
            .updateProfileById(lawyerId, update);
        if (!updated) {
            throw new httpException_1.HttpExceptoin(404, messages_1.MESSAGES.noUserWithId[LANGUAGE]);
        }
