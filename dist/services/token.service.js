...
class TokenService {
  ...
  invalidTokenException() {
    return new httpException_1.HttpException(
      401,
      messages_1.MESSAGES.unauthorized[env_1.env.LANGUAGE],
  ...
  invalidRefreshTokenException() {
    return new httpException_1.HttpException(
      401,
      messages_1.MESSAGES.invalidRefToken[env_1.env.LANGUAGE],
  ...
  async generateRefreshToken(userId) {
    ...
    const refreshTokenTTLSeconds =
      env_1.env.REFRESH_TOKEN_TTL_DAYS * 24 * 60 * 60;
  ...
  verifyAccessToken(token) {
    try {
      ...
      if (
        ...
        decoded.type !== "access" ||
        decoded.role !== lawyer_constants_1.LAWYER_ROLES.LAWYER
      ) {
  ...
  async consumeRefreshToken(refreshToken) {
    ...
    if (!stored) {
      throw new httpException_1.HttpException(
        401,
        messages_1.MESSAGES.noRefToken[env_1.env.LANGUAGE],
